...
cams_switch = {
    ...
    "Bottom": "rtsp://192.168.1.123:8554/video0_unicast"
}
IPS = {
    "Side": "rtsp://192.168.1.123:8554/video2_unicast",
    "Main": "rtsp://192.168.1.120:8554/video2_unicast", 
    "Gripper_L": "rtsp://192.168.1.123:8554/video6_unicast",
    "Gripper_R": "rtsp://192.168.1.123:8554/video4_unicast ",
...
class CoPilot:
    def __init__(self, window, geometry):
        ...
        self.dir = "/home/mahmoud/rov/colcon_ws/src/rov_24/rov_24"
...
def switch_cam():
    ...
    process = Process(
        ...
        args=(IPS["Switch"], "Switch", SIZE[4], POS[4], getScreensInfo(2)),
    )
    process.start()
    PROCESSES[4] = process
...
def main():
    for i in range(len(IPS)):
        process = Process(
            target=displayCamera,
            args=(list(IPS.values())[i], NAMES[i], SIZE[i], POS[i], getScreensInfo(2)),
        )
        PROCESSES[i] = process
        process.start()
    init_copilot()
